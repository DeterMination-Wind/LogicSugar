...
public final class ReconstructionMatrixTest{
    ...
    private static void addOperationFixtures(){
        ...
        addCarrier("op.array.sum", array + "datacall sum r \"buf\"\n", "datacall sum r");
        addCarrier("op.array.avg", array + "datacall avg r \"buf\"\n", "datacall avg r");
        addCarrier("op.array.min", array + "datacall min r \"buf\"\n", "datacall min r");
        addCarrier("op.array.max", array + "datacall max r \"buf\"\n", "datacall max r");
        addCarrier("op.array.count", array + "datacall count r \"buf, 1\"\n", "datacall count r");
        addCarrier("op.array.indexof", array + "datacall indexof r \"buf, 1\"\n", "datacall indexof r");
        addCarrier("op.array.fill", array + "datacall fill ~ \"buf, 0\"\n", "datacall fill ~");
        addCarrier("op.array.copy", "array dst cell1 0 4\narray src cell1 8 4\ndatacall copy ~ \"dst, src\"\n", "datacall copy ~");
        addCarrier("op.array.sortasc", array + "datacall sortasc ~ \"buf\"\n", "datacall sortasc ~");
        addCarrier("op.array.sortdesc", array + "datacall sortdesc ~ \"buf\"\n", "datacall sortdesc ~");
        addCarrier("op.array.reverse", array + "datacall reverse ~ \"buf\"\n", "datacall reverse ~");
        addCarrier("op.array.replace", array + "datacall replace r \"buf, 1, 2\"\n", "datacall replace r");
        addCarrier("op.array.swap", array + "datacall swap ~ \"buf, 0, 1\"\n", "datacall swap ~");
        addCarrier("op.array.bsearch", array + "datacall bsearch r \"buf, 1\"\n", "datacall bsearch r");

        String stack = "stack s cell1 0 4\n";
        addCarrier("op.stack.push", stack + "datacall spush r \"s, 1\"\n", "datacall spush r");
        addCarrier("op.stack.pop", stack + "datacall spop r \"s\"\n", "datacall spop r");
        addCarrier("op.stack.peek", stack + "datacall speek r \"s\"\n", "datacall speek r");
        addCarrier("op.stack.size", stack + "datacall ssize r \"s\"\n", "datacall ssize r");
        addCarrier("op.stack.clear", stack + "datacall sclear ~ \"s\"\n", "datacall sclear ~");

        String queue = "queue q cell1 0 4\n";
        addCarrier("op.queue.push", queue + "datacall qpush r \"q, 1\"\n", "datacall qpush r");
        addCarrier("op.queue.pop", queue + "datacall qpop r \"q\"\n", "datacall qpop r");
        addCarrier("op.queue.peek", queue + "datacall qpeek r \"q\"\n", "datacall qpeek r");
        addCarrier("op.queue.size", queue + "datacall qsize r \"q\"\n", "datacall qsize r");
        addCarrier("op.queue.clear", queue + "datacall qclear ~ \"q\"\n", "datacall qclear ~");

        String deque = "deque d cell1 0 4\n";
        addCarrier("op.deque.pushf", deque + "datacall dpushf r \"d, 1\"\n", "datacall dpushf r");
        addCarrier("op.deque.pushb", deque + "datacall dpushb r \"d, 2\"\n", "datacall dpushb r");
        addCarrier("op.deque.popf", deque + "datacall dpopf r \"d\"\n", "datacall dpopf r");
        addCarrier("op.deque.popb", deque + "datacall dpopb r \"d\"\n", "datacall dpopb r");
        addCarrier("op.deque.peekf", deque + "datacall dpeekf r \"d\"\n", "datacall dpeekf r");
        addCarrier("op.deque.peekb", deque + "datacall dpeekb r \"d\"\n", "datacall dpeekb r");
        addCarrier("op.deque.size", deque + "datacall dsize r \"d\"\n", "datacall dsize r");
        addCarrier("op.deque.clear", deque + "datacall dclear ~ \"d\"\n", "datacall dclear ~");

        String bitset = "bitset bits cell1 0 2\n";
        addCarrier("op.bitset.set", bitset + "datacall bset r \"bits, 1\"\n", "datacall bset r");
        addCarrier("op.bitset.clr", bitset + "datacall bclr r \"bits, 1\"\n", "datacall bclr r");
        addCarrier("op.bitset.test", bitset + "datacall btest r \"bits, 1\"\n", "datacall btest r");
        addCarrier("op.bitset.count", bitset + "datacall bcount r \"bits\"\n", "datacall bcount r");

        String map = "map m cell1 0 4\n";
        addCarrier("op.map.set", map + "datacall mapset r \"m, 1, 2\"\n", "datacall mapset r");
        addCarrier("op.map.get", map + "datacall mapget r \"m, 1\"\n", "datacall mapget r");
        addCarrier("op.map.has", map + "datacall maphas r \"m, 1\"\n", "datacall maphas r");
        addCarrier("op.map.del", map + "datacall mapdel r \"m, 1\"\n", "datacall mapdel r");
        addCarrier("op.map.size", map + "datacall mapsize r \"m\"\n", "datacall mapsize r");
        addCarrier("op.map.clear", map + "datacall mapclear ~ \"m\"\n", "datacall mapclear ~");

        String uset = "uset u cell1 0 4\n";
        addCarrier("op.uset.add", uset + "datacall uadd r \"u, 1\"\n", "datacall uadd r");
        addCarrier("op.uset.has", uset + "datacall uhas r \"u, 1\"\n", "datacall uhas r");
        addCarrier("op.uset.del", uset + "datacall udel r \"u, 1\"\n", "datacall udel r");
        addCarrier("op.uset.size", uset + "datacall usize r \"u\"\n", "datacall usize r");
        addCarrier("op.uset.clear", uset + "datacall uclear ~ \"u\"\n", "datacall uclear ~");

        String list = "list l cell1 0 8\n";
        addCarrier("op.list.append", list + "datacall lappend r \"l, 1\"\n", "datacall lappend r");
        addCarrier("op.list.get", list + "datacall lget r \"l, 0\"\n", "datacall lget r");
        addCarrier("op.list.set", list + "datacall lset r \"l, 0, 2\"\n", "datacall lset r");
        addCarrier("op.list.insert", list + "datacall linsert r \"l, 0, 2\"\n", "datacall linsert r");
        addCarrier("op.list.remove", list + "datacall lremove r \"l, 0\"\n", "datacall lremove r");
        addCarrier("op.list.find", list + "datacall lfind r \"l, 2\"\n", "datacall lfind r");
        addCarrier("op.list.size", list + "datacall lsize r \"l\"\n", "datacall lsize r");

        String heap = "heap h cell1 0 8\n";
        addCarrier("op.heap.push", heap + "datacall hpush r \"h, 1\"\n", "datacall hpush r");
        addCarrier("op.heap.pop", heap + "datacall hpop r \"h\"\n", "datacall hpop r");
        addCarrier("op.heap.size", heap + "datacall hsize r \"h\"\n", "datacall hsize r");

        String chain = "chain c cell1 0 8\n";
        addCarrier("op.chain.init", chain + "datacall cinit r \"c\"\n", "datacall cinit r");
        addCarrier("op.chain.clear", chain + "datacall cclear r \"c\"\n", "datacall cclear r");
        addCarrier("op.chain.new", chain + "datacall cnew r \"c\"\n", "datacall cnew r");
        addCarrier("op.chain.free", chain + "datacall cfree r \"c, 0\"\n", "datacall cfree r");
        addCarrier("op.chain.get", chain + "datacall cget r \"c, 0\"\n", "datacall cget r");
        addCarrier("op.chain.set", chain + "datacall cset r \"c, 0, 5\"\n", "datacall cset r");
        addCarrier("op.chain.next", chain + "datacall cnext r \"c, 0\"\n", "datacall cnext r");
        addCarrier("op.chain.link", chain + "datacall clink r \"c, 0, 1\"\n", "datacall clink r");
        addCarrier("op.chain.sethead", chain + "datacall cshead ~ \"c, 0\"\n", "datacall cshead ~");
        addCarrier("op.chain.head", chain + "datacall chead r \"c\"\n", "datacall chead r");
        addCarrier("op.chain.len", chain + "datacall clen r \"c\"\n", "datacall clen r");
    }
