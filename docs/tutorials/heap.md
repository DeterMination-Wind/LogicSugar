...
> 提示：编辑器菜单与积木显示的是新名字（如 `heap_push`）；旧短名（如 `hpush`）仍能解析已有存档，但新写的卡片一律用新名。

方法糖：`h.size()` / `h.length()` / `h.count()` 等价于 `heap_size(h)`。

没有「只看不弹」的 peek；如果需要保留最小值，可以在 `heap_pop` 后立刻把值 `heap_push` 回去（O(log n)）。
...
| 操作 | 复杂度 | 说明 |
| --- | --- | --- |
| `heap_push` / `heap_pop` | O(log n) | 堆高为 log n，向上/向下调整 |
| `heap_size` | O(1) | 读计数变量 |
