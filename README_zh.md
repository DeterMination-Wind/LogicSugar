...
- **数据结构** —— `matrix`（二维数组）、`arrayinit`、`record`、`stack` / `queue`、`bitset`、`map`（哈希表）、`list`、`heap`、`chain`（链表）等卡片把内存块的一段地址登记为结构化数据；`sum` / `avg` / `sortasc`、`spush` / `qpop`、`btest`、`mapset`、`lappend`、`hpush`、`cinit` / `cnew` 等表达式展开为普通原版指令。声明卡不进入保存的代码，重新打开自动折回，产物在任何原版客户端可运行。
