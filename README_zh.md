...
| **表达式语句** | 写 `result = (a + b) * 2` 一行搞定；保存时自动展开为等价指令，重新打开自动折叠回来，写错当场标红。 |
...
数组表达式里可直接写 `buf[i]`、`buf[i] = 5` 这样的下标读写；它们编译为普通原版 `read`、`write` 指令，重新打开自动折回表达式卡。

`fill`（整段同值初始化）、`sum`、`reverse`、`spush`、`qpop`、`dpushf`、`btest`、`mapset`、`uadd`、`lappend`、`hpush`、`cinit`、`cnew` 等 intrinsic 均以对应分类下的独立操作积木提供。旧八槽 `arrayinit` 只保留存档兼容。所有操作仍会降级为普通原版指令，并可通过 Sugar 载体重新打开。
