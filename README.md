...
- **Data structures** — `matrix` (2-D arrays), `arrayinit`, `record`, `stack` / `queue`, `bitset`, `map` (hash table), `list`, `heap` and `chain` cards name structured regions of memory blocks; expression operations such as `sum` / `avg` / `sortasc`, `spush` / `qpop`, `btest`, `mapset`, `lappend`, `hpush` and `cinit` / `cnew` expand to plain vanilla instructions. Declarations never enter the saved code, everything folds back on reopen, and the saved program stays vanilla-compatible.
